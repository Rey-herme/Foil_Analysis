...
int FitSpectraSkewGaussian(TH1D *h1, TCanvas * cFit, vector<vector<double>> *Peaks, int search = 5, double s = 0.005, double start = 3000, double end = 3600){
  ...
  TF1 *FitFunction = new TF1("FitFunction", fitfunctionskewgaussian, emin, emax, 4+npeaks*2);
  ...
  double peakpos, peakmax, sigma = 5, sigmamin = 5, sigmamax = 40, counts, width = 3*sigma;
...
int FitAllSkewGaussian(int Nfoil, double min = 3000, double max = 3600, bool V = false){
  ...
  FitSpectraSkewGaussian(hout,c1,&Peaksout,s,ss,min,max);
  if(V) cout << "Results in the foil : " << endl;
  TCanvas *c2 =  new TCanvas("c2", "c2", 800, 600);
  FitSpectraSkewGaussian(hfoil,c2,&Peaksfoil,s,ss,min,max);


  for(int i=0;i<Peaksout.size();i++){
    if(Peaksout[i][0]>6800 and Peaksout[i][0]<6900){
      Peaksout.erase(Peaksout.begin()+i);
    }
  }
  for(int i=0;i<Peaksfoil.size();i++){
    if(Peaksfoil[i][0]>6800 and Peaksfoil[i][0]<6900){
      Peaksfoil.erase(Peaksfoil.begin()+i);
    }
  }
  ...
  double PeaksoutCounts[5];
  double PeaksoutCountserr[5];
  ...
  double PeaksfoilCounts[5];
  double PeaksfoilCountserr[5];

  for(int i=0;i<5;i++){
    PeaksoutPos[i] = Peaksout[i][0];
    PeaksoutPoserr[i] = Peaksout[i][1];
    PeaksoutCounts[i] = Peaksout[i][4];
    PeaksoutCountserr[i] = Peaksout[i][5];
    PeaksfoilPos[i] = Peaksfoil[i][0];
    PeaksfoilPoserr[i] = Peaksfoil[i][1];
    PeaksfoilCounts[i] = Peaksfoil[i][4];
    PeaksfoilCountserr[i] = Peaksfoil[i][5];
  }
  ...
  double dx[5];
  double sdx[5];
  double SE[5] = {0.1731,0.1718,0.1708,0.1650,0.1641}; // Stopping power carbon
  if(Nfoil==50 or Nfoil==30){ // Stopping power Si3N4
    SE[0] = 0.2222;
    SE[1] = 0.2207;
    SE[2] = 0.2195;
    SE[3] = 0.2126;
  ...
  double PeaksRatio[] = {PeaksfoilCounts[0]/PeaksfoilCounts[2],PeaksfoilCounts[1]/PeaksfoilCounts[2],PeaksfoilCounts[3]/PeaksfoilCounts[4]};
  double PeaksRatioerr[] = {PeaksRatio[0]*TMath::Sqrt(PeaksfoilCountserr[0]*PeaksfoilCountserr[0]/(PeaksfoilCounts[0]*PeaksfoilCountserr[0])+PeaksfoilCountserr[2]*PeaksfoilCountserr[2]/(PeaksfoilCounts[2]*PeaksfoilCountserr[2])),
          PeaksRatio[1]*TMath::Sqrt(PeaksfoilCountserr[1]*PeaksfoilCountserr[1]/(PeaksfoilCounts[1]*PeaksfoilCountserr[1])+PeaksfoilCountserr[2]*PeaksfoilCountserr[2]/(PeaksfoilCounts[2]*PeaksfoilCountserr[2])),
          PeaksRatio[0]*TMath::Sqrt(PeaksfoilCountserr[3]*PeaksfoilCountserr[3]/(PeaksfoilCounts[3]*PeaksfoilCountserr[3])+PeaksfoilCountserr[4]*PeaksfoilCountserr[4]/(PeaksfoilCounts[4]*PeaksfoilCountserr[4]))};

  double Ratio[] = {1.66/84.45,13.23/84.45,23.3/76.7};
  double Ratioerr[] = {Ratio[0]*TMath::Sqrt(3.*3./(166.*166.)+10.*10./(8445.*8445.)),
           Ratio[1]*TMath::Sqrt(10.*10./(1323.*1323.)+10.*10./(8445.*8445.)),
           Ratio[2]*TMath::Sqrt(4.*4./(233.*233.)+4.*4./(767.*767.))};

  if(V){
    cout << "//////////////////////////////////////////////" << endl;
    cout << "Peaks relative intensities with the foil vs expected relative intensities : " << endl;
    cout << PeaksRatio[0] << " +/- " << PeaksRatioerr[0] << " pour " << Ratio[0] << " +/- " << Ratioerr[0] << endl;
    cout << PeaksRatio[1] << " +/- " << PeaksRatioerr[1] << " pour " << Ratio[1] << " +/- " << Ratioerr[1] << endl;
    cout << PeaksRatio[2] << " +/- " << PeaksRatioerr[2] << " pour " << Ratio[2] << " +/- " << Ratioerr[2] << endl << endl;;
  }
  if(V){
    cout << "//////////////////////////////////////////////" << endl;
