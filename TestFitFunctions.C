...
int FitAllSkewGaussian(int Nfoil, double min = 3000, double max = 3600, bool V = false){
  ...
  double Litt[] = {5105.81,5156.59,5485.56,5762.65,5804.77};
  double Litterr[] = {0.21,0.14,0.12, 0.03, 0.05};
  ...
  for(int i=0;i<8;i++){
    ...
    PeaksoutPos[i-c] = Peaksout[i][0];
    PeaksoutPoserr[i-c] = Peaksout[i][1];
    PeaksfoilPos[i-c] = Peaksfoil[i][0];
    PeaksfoilPoserr[i-c] = Peaksfoil[i][1];
  }
  ...
  double dx[5];
  double sdx[5];
  //double SE[5] = {0.1731,0.1718,0.1708,0.1650,0.1641};
  double SE[5] = {0.180,0.178,0.171,0.165,0.164};
  double sSE = 0.05;
  double w[5];
  double wE[5];
